...
# 注意这里改成 index_k
INDEX_ROOT = os.path.join(BASE_DIR, "outputs", "index_k")

# 我们这次只关心 enron_5k，且有 k=10,20,40 三套索引
DATASET = "enron_5k"
K_LIST = [10, 20, 40]
...
def _split_centroids(ddir: str, seed: int = 2025):
    ...
    if not os.path.exists(unit_path) or not os.path.exists(meta_path):
        raise FileNotFoundError(f"{unit_path} 或 {meta_path} 不存在，请先跑 build_index_enron_k.py")
...
def make_index_shares_for_dataset(dataset: str, k_value: int):
    """
    对 /root/siton-tmp/outputs/index_k/k{k_value}/{dataset} 目录下的索引做 shares 拆分
    """
    ddir = os.path.join(INDEX_ROOT, f"k{k_value}", dataset)
...
if __name__ == "__main__":
    for k in K_LIST:
        make_index_shares_for_dataset(DATASET, k)
    print("\n=== index_shares_k.py 完成 ✅ ===")
